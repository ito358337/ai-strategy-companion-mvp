...
- **案件一覧**: 複数案件の保存・切替・複製・削除。案件ごとに明細・設定・お客様情報を保持します。
- **入力**: 工区・部屋ごとに明細（数量・お客様単価・実行単価）を入力。手入力金額が単価×数量より優先されます。工事項目名が単価マスターと一致すると単価などを自動補完します。
- **お客様見積書**: 工区別の税抜・消費税（端数切り捨て）・税込を自動集計し、原価を含まない明細を表示。
- **実行予算 / 粗利分析**: 工区別・明細別の粗利額と粗利率を、目標粗利率に対して OK / 注意 / 要改善 で判定。
- **単価マスター**: 新規案件のひな形になる明細マスター（初期値はAetherサンプル156明細）をブラウザ上で編集。
- **CSV出力 / PDF出力**: 明細をExcelで開けるCSV（BOM付きUTF-8）で書き出し。表示中の帳票（見積書または実行予算書）は印刷ビューでPDF保存。
- データは `src/data/estimate-master.json` を初期値として、ブラウザのlocalStorageに保存されます。

## セッション履歴（経営伴走アプリ）

ヘッダーの「履歴」から、セッションの一覧・再開・閲覧ができます。複数セッションをブラウザ内に保存し、名称変更・削除・新規作成が可能です。
